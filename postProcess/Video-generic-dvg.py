...
def gettingFacets(filename, includeCoat='true'):
    ...
    exe = ["/home/soapy/postProcess/getFacet", filename] #, includeCoat]
...
def gettingfield(filename, zmin, zmax, rmax, nr):
    ...
    exe = ["/home/soapy/postProcess/getData", filename, str(zmin), str(0), str(zmax), str(rmax), str(nr)]
    p = sp.Popen(exe, stdout=sp.PIPE, stderr=sp.PIPE)
    stdout, stderr = p.communicate()
...
def main():
    ...
    parser.add_argument('--nGFS', type=int, default=3, #4000,
    ...
    parser.add_argument('--caseToProcess', type=str,
                       default='/home/soapy/simulationCases', #'../simulationCases/soapBubble-full',
